//
//  CalculatorButtonScientificRounded.swift
//  Ultimate_Calculator
//
...
struct CalculatorButtonScientificRounded: View {
    var button: CalculatorButton
    @EnvironmentObject var env: GlobalEnvironment
    @EnvironmentObject var viewRouter: ViewRouter
    var body: some View {
        Button(action: {
            self.env.handlekeypress(self.button)
        }) {
            Text(button.title)
            .font(.system(size: 30))
                .frame(width: self.buttonWidth3(button: button), height: ((UIScreen.main.bounds.width - 11 * 6) / 10) * 0.55)
                .foregroundColor(.white)
                .background(self.buttoncolor(viewRouter: viewRouter, button: button))
                .cornerRadius(self.buttonWidth3(button: button) / 4)
        }
    }
    ...
    private func buttonWidth3(button: CalculatorButton) -> CGFloat {
        return ((UIScreen.main.bounds.width - 11 * 6) / 10) * 0.9
    }
}
//struct CalculatorButtonScientificRounded_Previews: PreviewProvider {
//    static var previews: some View {
//        CalculatorButtonScientificRounded()
//    }
//}
